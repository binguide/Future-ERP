...
describe('User + argon2 (e2e)', () => {
  ...
  // `users` is tenant-scoped, so service calls outside an HTTP request must run
  // inside the tenant context (the same guarantee the middleware gives requests).
  ...
  it('creates a user with an argon2-hashed password', async () => {
    const user = await inTenant(() =>
      userService.create('ali@example.com', 'Ali Ahmed', 'MySecret123'),
    );
  ...
  it('finds user by email', async () => {
    const user = await inTenant(() => userService.findByEmail('ali@example.com'));
    expect(user).toBeDefined();
    expect(user!.email).toBe('ali@example.com');
  });
  ...
  it('validates correct password', async () => {
    const user = await inTenant(() =>
      userService.validatePassword('ali@example.com', 'MySecret123'),
    );
    expect(user).toBeDefined();
    expect(user!.email).toBe('ali@example.com');
  });
  ...
  it('rejects wrong password', async () => {
    const user = await inTenant(() =>
      userService.validatePassword('ali@example.com', 'WrongPassword'),
    );
    expect(user).toBeNull();
  });
  ...
  it('rejects unknown email', async () => {
    const user = await inTenant(() =>
      userService.validatePassword('nobody@example.com', 'anything'),
    );
    expect(user).toBeNull();
  });
  ...
  it('rejects duplicate email', async () => {
    await expect(
      inTenant(() => userService.create('ali@example.com', 'Duplicate', 'Pass123')),
